...
export default function Settings() {
  ...
  return (
    <div className="flex justify-center items-center h-dvh">
      <div
        className="transition-all bg-black/[.4] shadow-xl rounded-lg w-[90%]
        lg:w-[50%] xl:w-[40%] 2xl:w-[25%] min-w-[30vw] min-h-fit min-w-fit px-12 py-8"
      >
        ...
        <div className="flex flex-col gap-4 mt-2">
          <div className="flex justify-between">
            <span className="select-none">Fade In Background</span>
            <span
              className={`select-none transition
              ${!toggleSettings.dynamicBackground && "text-gray-300"}`}
            >
              {duration}ms
            </span>
          </div>
          <input
            className={`
              appearance-none w-full h-2 rounded-lg focus:outline-none
              accent-white transition-all duration-300
              ${!toggleSettings.dynamicBackground ? "bg-gray-300" : "bg-spotify"}
            `}
            type="range"
            step="50"
            min={minDuration}
            ...
            disabled={!toggleSettings.dynamicBackground}
          />
        </div>
      </div>
    </div>
